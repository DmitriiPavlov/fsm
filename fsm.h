...
fsm_state_t*  fsm_init_state(fsm_master_t*, uint32_t state_enum_id, void* data);
void fsm_register_handler(fsm_master_t* m, uint32_t state_enum_id, uint32_t event_enum_id, fsm_state_handler handler);
void fsm_register_default_handler(fsm_master_t* m, uint32_t state_enum_id, fsm_state_handler handler);
int  fsm_execute_one(fsm_master_t* m, fsm_state_t* state, fsm_event_t* event);
