...
#define _FSM_BEGIN_HANDLING(m, s, e) \
    fsm_master_t* ___fsm_internal_function_master_object = m; \
    fsm_state_t*  ___fsm_internal_function_state_object = s; \
    fsm_event_t*  ___fsm_internal_function_event_object = e;
#define FSM_BEGIN_HANDLING(m, s, e) _FSM_BEGIN_HANDLING((m), (s), (e))

#define FSM_STATE_IS(state_enum_literal) assert(__fsm_internal_function_state_object->enum_id == (state_enum_literal))
#define FSM_EVENT_IS(event_enum_literal) assert(__fsm_internal_function_event_object->enum_id == (event_enum_literal))
